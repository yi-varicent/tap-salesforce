...
When `source_type` is `report` then both `report_id` and `report_name` are required. we'll use report_name in the stream so it shows up properly on the Symon's UI
